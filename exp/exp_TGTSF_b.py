...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST, TGTSF_PatchTST
...
class Exp_Main(Exp_Basic):
    ...
    def _get_data(self, flag):
        data_loader = TGTSF_data_provider(self.args, flag, text_encoder=self.text_encoder)
        return data_loader
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_news, batch_des) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    if 'TGTSF' in self.args.model:
                        # compatability of PatchTST
                        # batch_news=batch_news.permute(0,2,3,1)
                        # print(batch_news.shape)
                        # batch_news = batch_news.unfold(dimension=-1, size=self.args.patch_len, step=self.args.stride)
                        # print(batch_news.shape)
                        # _, news_num, __, patch_num, patch_size = batch_news.shape
                        # batch_news = batch_news.reshape(_, patch_num, patch_size*news_num, __)
                        # print(batch_news.shape)
                        # print(batch_des.shape)
                        # batch_des = batch_des.unfold(dimension=1, size=self.args.patch_len, step=self.args.stride)
                        # print(batch_des.shape)
                        # batch_des = batch_des[:, :, :, :, -1] # only take the last day of the patch (can change)
                        ##################################
                        outputs = self.model(batch_x, batch_news, batch_des)
